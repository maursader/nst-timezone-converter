...
function safelyAccessChromeAPI(apiCall, callback) {
  try {
    ...
    isExtensionContextValid = false;
    console.warn('Chrome API error:', error.message);
    return false;
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  try {
    ...
    safelyAccessChromeAPI(
      (callback) => chrome.storage.sync.get('timezone', callback),
      function(data) {
  ...
  saveButton.addEventListener('click', function() {
    ...
    try {
      safelyAccessChromeAPI(
        (callback) => chrome.storage.sync.set({ 'timezone': selectedTimezone }, callback),
        function() {
          if (!isExtensionContextValid) {
            showContextError();
            return;
          ...
          try {
            safelyAccessChromeAPI(
              (callback) => chrome.tabs.query({url: '*://*.neopets.com/*'}, callback),
              function(tabs) {
                ...
                tabs.forEach(tab => {
                  try {
                    chrome.tabs.sendMessage(tab.id, { action: 'updateTimezone' });
                  } catch (error) {
